...
class RFM9x:
    ...
    low_datarate_optimize = _RegisterBits(_RH_RF95_REG_26_MODEM_CONFIG3, offset=3, bits=1) # required if symbol duration > 16ms
    ...
    def __init__(
        ...
        *,
        ...
        self._reset = GPIO.setup(12, GPIO.IN, pull_up_down=GPIO.PUD_UP)
            ...
            try:
                if self.rx_done():
                    ...
                    if self.crc_error():
                        ...
                        packet = self._read_into(_RH_RF95_REG_00_FIFO, packet)
    ...
    def _spi_write(self, register, payload):
        if type(payload) == int:
            ...
            payload = list(payload)  # Convert memoryview to bytes
    ...
    def _read_u8(self, address):
        ...
        return(self._read_into(address, self._BUFFER, length=1))
    ...
    def _write_from(self, address, buf, length=None):
        ...
        if length is None:
            length = len(buf)
        self._spi_write(address, buf)

    def _write_u8(self, address, val):
        # Write a byte register to the chip.  Specify the 7-bit address and the
        # 8-bit value to write to that address.
        self._spi_write(address,val)
        '''
    ...
    def reset(self):
        ...
        self._reset = GPIO.setup(12, GPIO.OUT)
    ...
    def receive(
        self, *, keep_listening=True, with_header=False, with_ack=False, timeout=None, debug=False, view=False, fast=False):
            ...
            if self.enable_crc and self.crc_error():
                ...
                if fifo_length < 5:
                    ...
                    # send ACK unless this was an ACK or a broadcast
                        ...
                        if debug: print('\t{}'.format(bytes(packet)))
    ...
    def receive_all(self, only_for_me=True,debug=False):
        ...
        if fifo_length > 0:
            ...
            _bigbuffer = self._read_into(_RH_RF95_REG_00_FIFO,_bigbuffer)
            for i in range(4):
